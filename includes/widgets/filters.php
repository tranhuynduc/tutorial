...
<form action="search.php" method="post">
    ...
    <input type="radio" name="price_sort" value="low"<?= (($price_sort == 'low')?' checked=checked': '') ?>>Low to High<br>
    <input type="radio" name="price_sort" value="high"<?= (($price_sort == 'high')?' checked=checked': '') ?>>High to Lowt<br><br>
    <input type="text" name="min_price" class="price-range" placeholder="Min $" value="<?= $min_price; ?>"> To
    <input type="text" name="max_price" class="price-range" placeholder="Max $" value="<?= $max_price; ?>"><br>
    <h4 class="text-center">Brand</h4>
    <input type="radio" name="brand" value="" <?= (($b =='')?' checked=checked': '') ;?>> All<br>
    <?php while($brand = mysqli_fetch_assoc($brandQ)) :?>
        <input type="radio" name="brand" value="<?= $brand['id'];?>" <?= (($b == $brand['id'])?' checked=checked': '') ;?>>
        <?= $brand['brand'];?><br>
    <?php endwhile; ?>
    <br>
